...
pub fn run(pattern: &str) -> Result<()> {
	...
	for (i, path) in paths.iter().enumerate() {
		...
		match inspect_model(path) {
			Ok(info) => {
				let print_result = |path: &Path, info: &ModelInfo| {
					print_inspect_result(path, info);
					if is_batch && !is_last {
						println!();
					}
				};
				...
				if let Some(ref pb) = pb {
					pb.suspend(|| print_result(path, &info));
				} else {
					if is_batch {
						println!(
							"Inspecting model {}/{}: {}",
...
fn print_inspect_result(path: &Path, info: &ModelInfo) {
	...
	tree_row(prefix, TREE_BRANCH, "Precision", label_w, &prec);
	...
	tree_row(prefix, tiling_connector, "Tiling", label_w, &tiling_str);
	...
	tree_row(prefix, TREE_LAST, "Ops", label_w, &ops_val);
